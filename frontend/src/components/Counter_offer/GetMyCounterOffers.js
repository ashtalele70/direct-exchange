...
import {
  Container, Row, Col, Button, Badge, Card, CardDeck, Alert
...
class GetMyCounterOffers extends Component {
  ...
  getMyCounterOffers = () => {
    ...
    axios
      .get(process.env.REACT_APP_ROOT_URL + "/getAllCounterOffers/" + this.offerId)
      .then((res) => {
        if (res.status === 200) {
  ...
  render() {
    ...
    displayform = (
      ...
      this.state.Offers_list.map((item, i) =>
        //{
        //return (
        //<div>
    ...
    return (
      ...
      <Container>
        ...
        {this.state.Offers_list.length == 0 && <Alert className="mt-5" variant="danger">No counter offers for this offer</Alert>}
        <CardDeck>
