...
import {Container, FormControl, Nav, Navbar, NavDropdown, Modal, Form, Button} from "react-bootstrap";
...
class Navigation extends Component {
    ...
    handleSubmit = () => {
        axios
        .put(process.env.REACT_APP_ROOT_URL + "/user?nickname=" + this.state.nickname + "&id=" + localStorage.getItem("userId"))
        .then((res) => {
          if (res.status === 200) {
        ...
        return (
            ...
            <Navbar bg="dark" variant="dark">
                <Navbar.Brand href="/">
                    ...
                    <Navbar.Brand>Hello, {localStorage.getItem("nickname")} 
                        <FontAwesomeIcon icon={faPen} className = "ml-1" onClick={() => this.setState({show: true})}/>
                    </Navbar.Brand>}
                    <Nav>
                ...
                <Modal.Body>
                        <Form>
                            <Form.Group controlId="formGroupNickname">
                                ...
                                <FormControl onChange={(e) => this.setState({ nickname: e.target.value })} />
